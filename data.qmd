...
```{r}
# install.packages(c("dplyr","tidyr","ggplot2","ggalluvial","forcats"))
library(dplyr) 
library(tidyr)
library(tidyverse)
library(ggplot2)
...
participation_df$total_participated <- rowSums(participation_df[, -1] == "participated", na.rm = TRUE)
```
...
ggplot(data = participation_df,
  ...
  scale_x_discrete(limits = c("2018", "2019", "2020", "2021"), 
                   expand = c(0.15, 0.05)) +
  theme_minimal() +
