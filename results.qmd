...
pisa_gdp2018 <- inner_join(
  read.csv("data/2018_GDP.csv"),
  read.csv("data/2018_PISA.csv"),
  by = "Country"
) %>%
  select(Country, Score, GDP) %>%
  mutate(
...
pisa_points <- pisa_gdp2018 %>%
  filter(Country != "OECD average")

oecd_avg_score <- pisa_gdp2018 %>% 
  filter(Country == "OECD average") %>% 
  pull(Score)

mean_gdp <- pisa_points %>% 
  summarize(mean_gdp = mean(GDP, na.rm = TRUE)) %>% 
  pull(mean_gdp)

pisa_points <- pisa_points %>%
  mutate(
    Above_Both = Score > oecd_avg_score & GDP > mean_gdp
  )


# --- Improved Plot ---
ggplot(pisa_points, aes(x = GDP, y = Score)) +
  ...
  geom_vline(xintercept = mean_gdp, 
  ...
  coord_cartesian(
    clip = "off",
    xlim = c(min(pisa_points$GDP) * 0.95, max(pisa_points$GDP) * 1.05),
    ylim = c(min(pisa_points$Score) * 0.97, max(pisa_points$Score) * 1.03)
  ) +

  labs(
    title = "PISA 2022 Reading Score vs GDP per Capita",
    subtitle = "Countries above OECD average reading score AND above mean GDP are highlighted",
...
pisa_gdp2022 <- inner_join(
  read.csv("data/2022_GDP.csv"),
  read.csv("data/2022_PISA.csv"),
  by = "Country"
) %>%
  select(Country, Reading_Mean, GDP) %>%
  mutate(
    Reading_Mean = as.numeric(Reading_Mean),
    GDP = as.numeric(GDP)
  )

pisa_points <- pisa_gdp2022 %>%
  filter(Country != "OECD average")

oecd_avg_score <- pisa_gdp2022 %>% 
  filter(Country == "OECD average") %>% 
  pull(Reading_Mean)
...
pisa_points <- pisa_points %>%
  mutate(
    Above_Both = Reading_Mean > oecd_avg_score & GDP > mean_gdp
  )


# --- Improved Plot ---
ggplot(pisa_points, aes(x = GDP, y = Reading_Mean)) +
  ...
  coord_cartesian(
    clip = "off",
    xlim = c(min(pisa_points$GDP) * 0.95, max(pisa_points$GDP) * 1.05),
    ylim = c(min(pisa_points$Reading_Mean) * 0.97, max(pisa_points$Reading_Mean) * 1.03)
  ) +
